...
- [x] Restore visible Product and variant status controls at the end of their forms, with a direct Product-tile status shortcut. Group exact-variant fulfilment into its own editor section, make every variant preview control open only its matching section with Done at that section's bottom, and make Blueprint/unlock actions synchronise the selected Product variant ID before opening a clearly labelled, variant-scoped connection workspace. Highlight unfilled Variant details, Descriptions, Blueprints, Unlocks, Visibility, Price and sale, Promotion videos, Prerequisites, and Bundle controls in purple. The variant preview now follows the public Product detail hierarchy and uses the effective long description; the public detail page also prefers the Product long description over a variant short description when no variant long-description override exists.
