...
- [x] Complete nested Create and attach workflows for missing reusable Items, Blueprints, and Plans while retaining the existing Asset drawer. Preserve the unsaved parent draft, create the child in the same Content Builder, return to the exact selector/variant, and attach the saved child without losing Product or entity edits. Required linked-record validation is handled by the visible selector; nested reusable records save without incorrectly creating a second Product relationship; a new manufacturing or Workshop Operations Blueprint is written directly back to the exact Product-variant connection that launched it; named template fields receive the new child ID and auto-save an existing parent record; and the nested Template and Content Builder drawers use explicit save actions rather than hidden browser-form validation.
