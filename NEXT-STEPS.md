...
- [x] Add operational state cues to Product previews: archived, hidden, coming-soon, draft, paused, and out-of-stock watermarks, plus a green ring for active Products/variants. The main tile shows Out of Stock only when every currently sellable tracked variant has no stock, displays Product type in the live badge position and category as its Marketplace filter, and keeps Product type/category controls hidden until selected. Variant Blueprint and unlock panels now retain all saved connections while showing only the active variant's relevant rows, and Add Product variant sits after those shared connection panels.
