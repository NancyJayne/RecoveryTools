...
- [x] Replace the mismatched Connections-step form previews with one interactive entity-relationship diagram. Keep the current Item, Blueprint, Plan, or Campaign in the centre with its active entity variants; show the configured Product preview, exact Product stock, template-enabled Item stock, bundle components, main-entity Assets, manufacturing Blueprints, Workshop operations, purchase unlocks, future Library selections, and reusable Item/Plan links as connected data-model nodes. Reuse the Product and Asset drawers, move the existing authoritative entity-stock controls into an Item-stock drawer without changing their fields or persistence, and keep Product stock separate through Stocktake.
...
- [x] Simplify the Connections ERD presentation: use a wider desktop canvas, arrange Product-owned child records in a two-column grid, allow headings and action labels to wrap, remove redundant explanatory paragraphs, and show concise entity/Product stock titles followed by exact variant levels.
- [ ] When the Marketing system exists, add Marketing as an outward ERD connection for the relevant entity and Product variants. Do not add placeholder Marketing records before their authoritative workflow and data model exist.
