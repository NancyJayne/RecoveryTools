...
- [x] Replace the mismatched Connections-step form previews with one interactive entity-relationship diagram. Keep the current Item, Blueprint, Plan, or Campaign in the centre with its active entity variants; show the configured Product preview, exact Product stock, template-enabled Item stock, bundle components, main-entity Assets, manufacturing Blueprints, Workshop operations, purchase unlocks, future Library selections, and reusable Item/Plan links as connected data-model nodes. Reuse the Product and Asset drawers and open Stocktake filtered to the current entity.
