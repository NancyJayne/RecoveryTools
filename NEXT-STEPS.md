...
- [x] Refine the Product editor into compact contextual sections: Product fulfilment displays its selected requirements without a count, each Product variant remains visible as a detail-page preview, preview and compact option controls open only the relevant fields, and Done closes the active editor. Marketplace visibility and price/sale stay hidden until selected, and Product inventory tracking no longer overwrites the independent Requires shipping choice.
