...
- [x] Restore visible Product and variant status controls at the end of their forms, with a direct Product-tile status shortcut. Group exact-variant fulfilment into its own editor section, make every variant preview control open only its matching section with Done at that section's bottom, highlight an unset affiliate price in purple, and make Blueprint/unlock actions synchronise the selected Product variant ID before opening a clearly labelled, variant-scoped connection workspace.
