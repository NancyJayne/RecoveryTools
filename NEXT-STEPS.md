...
- [x] Replace template-linked Item, Blueprint, Plan, and Asset dropdowns with one reusable selector modal. Apply template-defined table/type/status/tag constraints first, then allow name, tag, and type filtering; preserve repeatable-field limits, open prefilled child creation in a safe new tab, and refresh results without losing the parent entity. Use the same selector for exact Product-variant manufacturing Blueprint connections.
