...
- [x] Replace the mismatched Connections-step form previews with one interactive entity-relationship diagram. Keep the current Item, Blueprint, Plan, or Campaign in the centre with its active entity variants; show Product variants, exact stock, bundle components, main-entity Assets, manufacturing Blueprints, Workshop operations, purchase unlocks, and reusable Item/Plan links as connected data-model nodes. Reuse the Product and Asset drawers and open Stocktake filtered to the current entity.
- [ ] Extend the Connections ERD child workflow so connected Items, Plans, manufacturing Blueprints, and Workshop-operations Blueprints can be created or edited in nested drawers and returned to the exact node/variant. Until parent-draft restoration is complete, open Blueprints in a separate tab and keep reusable-link edits read-only in the ERD.
