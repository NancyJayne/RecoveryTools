...
- [x] Complete nested Create and attach workflows for missing reusable Items, Blueprints, and Plans while retaining the existing Asset drawer. Preserve the unsaved parent draft, create the child in the same Content Builder, return to the exact selector/variant, and attach the saved child without losing Product or entity edits. Required linked-record validation is handled by the visible selector, and a manufacturing Blueprint created from a Product can save before it is attached back to that Product.
