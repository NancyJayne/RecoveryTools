...
export function canonicalOrderLine(item, index, currency = "AUD") {
  ...
  return {
    ...
    relatedWorkshopId: clean(item.relatedWorkshopId),
    eventStartAt: clean(item.eventStartAt),
    eventEndAt: clean(item.eventEndAt),
    eventLocation: clean(item.eventLocation),
    instructorId: clean(item.instructorId),
    instructor: clean(item.instructor),
    sku: clean(item.sku),
    quantity,
