...
export const upsertAdminAsset = onCall(
  ...
  async (request) => {
    ...
    batch.set(ref, {
      ...
      externalProvider: clean(data.externalProvider, 50).toLowerCase(),
      embedUrl: clean(data.embedUrl, 4000),
      sourceType: clean(data.sourceType, 30).toLowerCase() || (data.storagePath ? "upload" : "external"),
